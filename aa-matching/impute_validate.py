...
#   DESCRIPTION:    Validation of nearest 10 vote imputation
#   OUTPUT:
#   DATE:           November 05, 2020
#   LATEST UPDATE:  January 07, 2021
#   UPDATE REASON:  Automate correction to coordinate system
...
#   NOTES:          Comparing results of imputation with later database ver.
###############################################################################

import pandas as pd
import numpy as np
import nuc_match_val as nuc_mm
import aa_match_val as aa_mm
...
def nearest10(loc, disFrame, rPos):
    ...
    with open("./data/validation/"+dbver+"/nearest/" + loc + "_topten.txt", "w+") as handle:
...
def impute(dbver, loc, locDict, refseq, aaDict):
    ...
    if recompute:
        ...
        disFrame.to_csv("./data/validation/"+dbver+"/locdist/" + loc + ".csv")
        ...
        disFrame = pd.read_csv("./data/validation/"+dbver+"/locdist/" + loc + ".csv", index_col=0)
    ...
    #FIXME (gbiagini) - currently only returning nucleotide sequence
    return locDict
...
for dbver in ["300", "350", "3100", "3200", "3300"]:
    nuc_mm.main(dbver)
    aa_mm.main(dbver)
    ...
    for loc in nuc_mm.refseq:
        ...
        locDict = newDict
        del (newDict)
        imputed = impute(dbver, loc, locDict, refseq[loc], aaDict)
        ...
        repFrame.to_csv('./data/validation/'+dbver+'/imputed/'+loc+'_imputed.csv', index=True)
