...
def retrieve_msf(dbversion, locus):
    ...
    if path.exists(seq_filename) == False:
        ...
        print("MSF file for locus " + locus + " already downloaded")
    return seq_filename
...
for locus in loci:
    loc_full_alseq = {}
    seq_filename = retrieve_msf(dbversion, locus)
    ...
    for record in multipleseq:
        ...
        loc_full_alseq[loc_full_allele] = SeqRecord(full_protein)
        ...
        if loc_full_allele == refseq[loc]:
            new_end, new_end_inc = adjust_offset(loc, ard_start_pos[loc], ard_start_pos_incomplete[loc],
                                                 ard_end_pos[loc], ard_end_pos_incomplete[loc], prev=0, prev_inc=0)
    ...
    # !GB!# Commented out these two lines as well as the generate_IMGT() function call, since the process is
    # intensive and does not need to be run every time the code is used. Might should consider a switch to
    # splitting a HLA_full_alseq DataFrame by locus and doing all of the work in a single function definition.
    # loc_full_alseq = remove_ins(loc_full_alseq) HLA_full_alseq.update(loc_full_alseq)
